...
//  ChatApp
//
//  Created by Caner Karabulut on 14.11.2023.
//
...
class ChatViewController: UICollectionViewController {
    ...
    private func fetchMessages() {
        UserService.fetchMessages(user: user) { messages in
...
extension ChatViewController: ChatInputViewProtocol {
    func sendMessage(_ chatInputView: ChatInputView, message: String) {
        ...
        UserService.sendMessage(message: message, toUser: user) { error in
            if let error = error {
                print("Error: \(error.localizedDescription)")
                return
            }
        }
        chatInputView.clearMessage()
    }
}
